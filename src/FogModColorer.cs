...
namespace FogMod
{
    public partial class FogMod : Mod
    {
        ...
        {
            float fadeIn = MathHelper.Clamp(p.AgeSeconds / SmokeFadeInSeconds, 0f, 1f);
            fadeIn = fadeIn * fadeIn * (3f - 2f * fadeIn);
            float fadeToFloorT = MathHelper.Clamp(p.AgeSeconds / SmokeFadeToFloorSeconds, 0f, 1f);
        ...
        {
            float cellSize = Math.Max(1f, FogTileSize);
            ...
            float jitter = Hash01(col * 73856093 ^ row * 19349663);
            float phase = breathBasePhase + jitter * MathHelper.TwoPi * BreathDesync;
            float s = (float)Math.Sin(phase);
            // Map to [1 - A, 1 + A], then clamp to [0, 1.5] and finally to [0,1]
            float mult = 1f + BreathAmplitude * s;
        ...
        private Color ApplyExplosionTintPerBlasts(Color inputColor, Vector2 worldPosition)
        {
            ...
            foreach (var flash in explosionFlashInfos)
            {
                ...
                float timeRatio = flash.TimeLeft / ExplosionFlashDurationSeconds;
